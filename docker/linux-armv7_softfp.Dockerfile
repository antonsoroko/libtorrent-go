FROM elementumorg/cross-compiler:linux-armv7_softfp
...
ENV BOOST_OPTS cxxflags=-fPIC cflags=-fPIC -march=armv7-a -mfloat-abi=softfp
RUN ./build-boost.sh
...
ENV LT_CXXFLAGS -std=c++11 -Wno-psabi -march=armv7-a -mfloat-abi=softfp
